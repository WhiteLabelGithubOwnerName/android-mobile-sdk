...
Colors can be modified by passing a JSON object to the `WtestSdk` instance. You can either completely override the theme or only change certain colors.

- `WtestSdk.instance?.setLightTheme(JSONObject)` allows to modify the payment dialog's light theme.
- `WtestSdk.instance?.setDarkTheme(JSONObject)` allows to modify the payment dialog's dark theme.
- `WtestSdk.instance?.setCustomTheme(JSONObject, ThemeEnum)` allows to enforce a specific theme (dark, light or your own).

```kotlin
// ...
import io.github.whitelabelgithubownername.enums.ThemeEnum
...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        WtestSdk.instance?.setDarkTheme(getCustomTheme())
        WtestSdk.instance?.setLightTheme(getCustomTheme())
    }
...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        WtestSdk.instance?.setCustomTheme(getCustomTheme(), ThemeEnum.DARK)
    }
...
```kotlin
  //...
WtestSdk.instance?.setCustomTheme(null, ThemeEnum.DARK)
```
...
Use `setAnimation` method to change the screen change animation. Currently avaliable options are: `AnimationEnum.SLIDE` and `AnimationEnum.BUBBLE`. Default value is `AnimationEnum.SLIDE`. `WtestSdk.instance?.setAnimation(AnimationEnum.BUBBLE)` allows to modify the payment dialog's dark theme. ![Slide Animation](../imgs/slideAnimation.gif) ![Bubble Animation](../imgs/bubbleAnimation.gif)
